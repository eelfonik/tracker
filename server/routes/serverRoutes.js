...
router
  ...
  .post(function(req, res) {
    ...
    const apiResponseStep1 = accountController.getUserFromUserRegistration(
      userSignup
    );
    ...
    if (apiResponseStep1.success) {
      accountController.signup(apiResponseStep1.extras.user, function(
        err,
        apiResponseStep2
      ) {
...
router
  ...
  .post((req, res) => {
    ...
    accountController.login(userLogin.email, userLogin.password, (err,response) => res.send(response));
  })
...
router.route("/account/resetpassword").post(function(req, res) {
  ...
  accountController.resetPassword(userPasswordReset.email, function(
    err,
    response
  ) {
...
router
  ...
  .get(function(req, res) {
    ...
    invoiceController.getSingleInvoice(invoiceNumber, function(
      error,
      response
    ) {
      res.send(response);
  ...
  .post(function(req, res) {
    ...
    const apiResponseGetNewInvoice = invoiceController.getInvoiceFromUserInput(
      invoiceInfo
    );
    res.set("Access-Control-Allow-Origin", "http://localhost:5000"); // Enable CORS in dev environment.
    ...
    if (apiResponseGetNewInvoice.success) {
      invoiceController.addNewInvoice(
        apiResponseGetNewInvoice.extras.invoice,
        function(err, apiResponseAddNewInvoice) {
