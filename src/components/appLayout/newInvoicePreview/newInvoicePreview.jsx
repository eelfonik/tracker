...
class NewInvoicePreview extends React.Component {
    ...
    if (!!this.props.data.description) {
      return (
        <div className={newInvoicePreviewStyle.incomeDesc}>
          <span className={newInvoicePreviewStyle.smallHeader}>
            Designation
          </span>
          <hr />
  ...
  taxIsZeroOrUndefined() {
    return (
      parseFloat(this.props.data.taxRate) === 0 ||
  ...
  calcTva() {
    if (!this.taxIsZeroOrUndefined()) {
      return parseFloat(
        (
          parseFloat(this.props.data.sum) *
          parseFloat(this.props.data.taxRate) /
          100
        ).toFixed(2)
      );
    }
  ...
  maybeRenderAdditionalTaxText() {
    if (this.taxIsZeroOrUndefined()) {
      ...
      return (
        <div className={newInvoicePreviewStyle.taxAdditionalText}>
          TVA {this.props.data.taxRate}% : {this.calcTva()}{" "}
  ...
  renderTotal() {
    const total = this.taxIsZeroOrUndefined()
      ? this.calcSum()
      : this.calcSum() + this.calcTva();
    return (
      <div>
        ...
        <div className={newInvoicePreviewStyle.invoiceWrapper}>
          <div className={newInvoicePreviewStyle.invoiceHeader}>
            <div className={newInvoicePreviewStyle.headerInfo}>
              <div className={newInvoicePreviewStyle.header}>
                {this.props.name}
              </div>
...
NewInvoicePreview = connect(mapStateToProps)(NewInvoicePreview);
